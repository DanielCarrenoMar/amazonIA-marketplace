...
export async function getExplorerProposals(): Promise<ProposalSummary[]> {
  // Errors (network, auth, 5xx) propagate to the caller instead of being
  // swallowed here — otherwise a real failure is indistinguishable from an
  // empty list of proposals.
...
export async function getExplorerProposalById(id: string): Promise<ProposalDetail | null> {
  // The backend returns null (not a 404) for an unknown id, so a genuinely
  // missing proposal already resolves normally — no need to swallow errors
  // here; let network/auth failures propagate to the caller.
