...
│  [Evidently AI] (data drift + prediction drift reports)                 │
...
async def obtener_pronostico_open_meteo(
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            # 2. Fallback: intentar NASA POWER histórico para el mismo punto
            #    (datos del año pasado como proxy climatológico)
            data = await _fallback_nasa_power_climatologico(lat, lon)
            if data is None:
                raise ClimateDataUnavailableError(
                    f"Open-Meteo y NASA POWER fallaron para ({lat},{lon})"
                ) from exc
...
async def _fallback_nasa_power_climatologico(lat: float, lon: float) -> Optional[dict]:
    """
    Fallback: datos históricos del año pasado desde NASA POWER como proxy climático.
    Solo se usa cuando Open-Meteo no está disponible.
    """
    hoy = date.today()
    hace_1_year = hoy.replace(year=hoy.year - 1)
    params = {
        "parameters": "PRECTOTCORR,T2M_MAX,T2M_MIN,RH2M,WS2M",
        "community":  "RE",
        "longitude":  lon,
        "latitude":   lat,
        "start":      hace_1_year.strftime("%Y%m%d"),
        "end":        (hace_1_year + timedelta(days=7)).strftime("%Y%m%d"),
        "format":     "JSON",
    }
    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
            ...
            return {
                ...
                "temperatura_max_c":   list(data["T2M_MAX"].values()),
                "temperatura_min_c":   list(data["T2M_MIN"].values()),
                "humedad_relativa_pct": list(data["RH2M"].values()),
                "velocidad_viento_ms": list(data["WS2M"].values()),
                "prob_precipitacion_pct": [None] * 7,   # NASA no provee probabilidad
                "evapotranspiracion":  [None] * 7,
                "fuente":             "nasa-power-fallback",
                "fecha_consulta":     date.today().isoformat(),
            }
        except Exception:
            return None
...
async def obtener_telemetria_envio(
    ...
    # Capa 2: Promedio histórico de HOY del sensor en MongoDB
    ...
    pipeline = [
        ...
        {"$group": {
            ...
            "humedad_media": {"$avg": "$humedad_pct"},
            "count": {"$sum": 1}
        }}
    ]
    resultado = await mongo_client["telemetria"]["iot_readings"].aggregate(pipeline).to_list(1)
    if resultado and resultado[0]["count"] >= 3:
        return TelemetriaIoT(
            temp_interna_c=resultado[0]["temp_media"],
            humedad_interna_pct=resultado[0]["humedad_media"],
...
class ResultadoSegmento(BaseModel):
    segmento_idx:   int
    coordenadas:    dict
    score_riesgo_pct: float
    nivel_alerta:   Literal["VERDE", "AMARILLO", "ROJO"]
    razones:        list[RazonRiesgo]
    fuente_clima:   str
    fuente_iot:     str
...
class RespuestaRiesgo(BaseModel):
    ...
    segmento_critico:      Optional[ResultadoSegmento]
    desglose_segmentos:    list[ResultadoSegmento]
...
async def evaluar_riesgo(
    ...
    # ── 3. Construir features y evaluar por segmento ──────────────
    resultados_segmentos = []
    for i, (punto, clima) in enumerate(zip(puntos_ruta, climas)):
        features = construir_features(solicitud, clima, telemetria, punto)
        ...
        nivel = _score_a_nivel(score_ajustado)
        resultados_segmentos.append(ResultadoSegmento(
            segmento_idx=i + 1,
            coordenadas={"lat": punto["lat"], "lon": punto["lon"]},
            score_riesgo_pct=round(score_ajustado * 100, 1),
            nivel_alerta=nivel,
            razones=[RazonRiesgo(**r) for r in prediccion["razones"]],
            fuente_clima=clima["fuente"],
            fuente_iot=telemetria.fuente.value,
        ))

    # ── 4. Regla del Eslabón Más Débil ────────────────────────────
    segmento_critico = max(resultados_segmentos, key=lambda s: s.score_riesgo_pct)
    score_final      = segmento_critico.score_riesgo_pct
    nivel_final      = segmento_critico.nivel_alerta
    latencia_ms      = round((time.monotonic() - t_inicio) * 1000, 1)

    respuesta = RespuestaRiesgo(
        envio_id=solicitud.envio_id,
        score_compuesto_pct=score_final,
        nivel_alerta=nivel_final,
        mensaje=_nivel_a_mensaje(nivel_final, segmento_critico.segmento_idx),
        razones_principales=segmento_critico.razones,
        segmento_critico=segmento_critico,
        desglose_segmentos=resultados_segmentos,
        metadata={
            "version_modelo": app.version_modelo,
            "latencia_ms":    latencia_ms,
            "fuente_iot":     telemetria.fuente.value,
            "confianza_iot":  telemetria.confianza,
            "n_segmentos":    len(resultados_segmentos),
        }
    )
